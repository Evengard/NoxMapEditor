...
// Desc:
//-----------------------------------------------------------------------------
HRESULT CDisplay::CreateWindowedDisplay( HWND hWnd, DWORD dwWidth, DWORD dwHeight,bool Clipped)
{
    HRESULT hr;
...
*/    LPDIRECTDRAWCLIPPER pcClipper,pcClipper2;
    ...
    if( FAILED( hr = m_pDD->CreateSurface( &ddsd, &BackGroundBuffer, NULL ) ) )
        return E_FAIL;
    ...
    if( FAILED( hr = m_pDD->CreateClipper( 0, &pcClipper2, NULL ) ) )
        return E_FAIL;
 

    //Attach_Clipper(dwWidth,dwHeight)
RECT rec;
CLIPLIST ClipList;
...
rec.left=0;
rec.top =0;
rec.right = dwWidth-50;
rec.bottom = dwHeight-50;
...
memcpy(&ClipList.rgndata, &rec, sizeof(RECT));
  ...
  if(!Clipped)
  {
    ...
    if( FAILED( hr = pcClipper2->SetClipList((LPRGNDATA)&ClipList,0)))
	{
       pcClipper2->Release();
       return hr;
    }
  ...
  else
  {
     ...
     if( FAILED( hr = m_pddsBackBuffer->SetClipper( pcClipper ) ) )
    {
     ...
     if( FAILED( hr = m_pddsBackBuffer->SetClipper( pcClipper2 ) ) )
    {
        pcClipper2->Release();
        return E_FAIL;
    }
...
// Desc: 
//-----------------------------------------------------------------------------
HRESULT CDisplay::Blt_Trans( DWORD x, DWORD y, LPDIRECTDRAWSURFACE7 pdds, RECT* prc,
                       DWORD dwFlags, int Alpha)
{
    if( NULL == m_pddsBackBuffer )
        return E_POINTER;
	...
	prc->right+=x;
*/
    return m_pddsBackBuffer->Blt(prc,pdds,NULL, dwFlags | DDBLT_ALPHASRCCONSTOVERRIDE | DDBLT_ALPHASRC,&ddbltfx);
...
// Desc: 
//-----------------------------------------------------------------------------
HRESULT CDisplay::Blt( DWORD x, DWORD y, DWORD Width, DWORD Height, LPDIRECTDRAWSURFACE7 pdds, RECT* prc,
                       DWORD dwFlags )
{
    if( NULL == m_pddsBackBuffer )
        return E_POINTER;
	...
	if(prc)
	{
      return m_pddsBackBuffer->Blt(&ppr,pdds,prc,dwFlags,NULL);
	}
	else
	{
	...
	{
    return m_pddsBackBuffer->Blt(NULL,pdds,NULL,dwFlags,NULL);
	}
	else
	{
    return m_pddsBackBuffer->Blt(&ppr,pdds,/*&prc2*/NULL,dwFlags,NULL);
	}
...
// Desc: 
//-----------------------------------------------------------------------------
HRESULT CDisplay::Blt_ColorRec( DWORD x, DWORD y,DWORD Width,DWORD Height, LPDIRECTDRAWSURFACE7 pdds,DWORD dwColor)
{
	RECT ppr;
	ppr.left=x;
	ppr.right=x+Width;
 ...
 return m_pddsBackBuffer->Blt( &ppr, NULL, NULL, DDBLT_COLORFILL, &ddbltfx );
...
// Desc: 
//-----------------------------------------------------------------------------
HRESULT CDisplay::Blt( DWORD x, DWORD y, DWORD Width, DWORD Height, CSurface* pSurface, RECT* prc )
{
    if( NULL == pSurface )
        return E_INVALIDARG;
    ...
    if(Width==(-1) || Height==(-1))
	{
	  LPDDSURFACEDESC2 desc=pSurface->GetDDrawDesc();
	  Width=desc->dwWidth;
	  Height=desc->dwHeight;
	}

    if( pSurface->IsColorKeyed() )
        return Blt( x, y, Width, Height, pSurface->GetDDrawSurface(), prc, DDBLT_KEYSRC);
    else
        return Blt( x, y, Width, Height, pSurface->GetDDrawSurface(), prc, 0L );
}
...
// Desc: 
//-----------------------------------------------------------------------------
HRESULT CDisplay::Blt_Fast( DWORD x, DWORD y, LPDIRECTDRAWSURFACE7 pdds, RECT* prc,
                       DWORD dwFlags )
{
    if( NULL == m_pddsBackBuffer )
        return E_POINTER;
...
// Desc: 
//-----------------------------------------------------------------------------
HRESULT CDisplay::BltFast( DWORD x, DWORD y, CSurface* pSurface, RECT* prc )
{
    if( NULL == pSurface )
        return E_INVALIDARG;

    if( pSurface->IsColorKeyed() )
        return Blt_Fast( x, y, pSurface->GetDDrawSurface(), prc, DDBLTFAST_SRCCOLORKEY);
    else
        return Blt_Fast( x, y, pSurface->GetDDrawSurface(), prc, 0L);
}
...
// Desc: 
//-----------------------------------------------------------------------------
HRESULT CDisplay::Blt_Trans( DWORD x, DWORD y, CSurface* pSurface, RECT* prc, int Alpha )
{
    if( NULL == pSurface )
        return E_INVALIDARG;

    if( pSurface->IsColorKeyed() )
        return Blt_Trans( x, y, pSurface->GetDDrawSurface(), prc, DDBLTFAST_SRCCOLORKEY,Alpha);
    else
        return Blt_Trans( x, y, pSurface->GetDDrawSurface(), prc, 0L,Alpha);
}
...
// Desc: 
//-----------------------------------------------------------------------------
HRESULT CDisplay::UpdateBounds(int &WINDOW_WIDTH,int &WINDOW_HEIGHT)
{
    if( m_bWindowed )
    {
        ...
        ClientToScreen( m_hWnd, (POINT*)&m_rcWindow+1 );
		  ...
		  WINDOW_WIDTH=m_rcWindow.right;
	   	  WINDOW_HEIGHT=m_rcWindow.bottom;
    }
    else
    {
...
//       bitmap if nessasary
//-----------------------------------------------------------------------------
HRESULT CSurface::DrawBitmapEx( TCHAR *strBMP, 
                              DWORD dwBMPOriginX, DWORD dwBMPOriginY, 
                              DWORD dwBMPWidth, DWORD dwBMPHeight ,DWORD DesiredWidth, DWORD DesiredHeight , int xOffset)
{
    ...
    if( strBMP == NULL) 
     ...
     // Try to load the bitmap as a resource, if that fails, try it as a file
    hBMP = (HBITMAP) LoadImage( GetModuleHandle(NULL), strBMP, 
                                IMAGE_BITMAP, dwBMPWidth, dwBMPHeight, 
                                LR_CREATEDIBSECTION );
    if( hBMP == NULL )
    {
        hBMP = (HBITMAP) LoadImage( NULL, strBMP, 
...
//       GDI font if hFont is NULL.  
//-----------------------------------------------------------------------------
HRESULT CDisplay::DrawText( HFONT hFont, TCHAR* strText, 
                            ...
                            COLORREF crBackground, COLORREF crForeground )
{
    ...
    // Make sure this surface is restored.
    if( FAILED( hr = m_pddsBackBuffer->Restore() ) )
        return hr;
...
// Desc: 
//-----------------------------------------------------------------------------
HRESULT CSurface::Blt( DWORD x, DWORD y,DWORD Width,DWORD Height,LPDIRECTDRAWSURFACE7 pdds, RECT* prc,
                       DWORD dwFlags )
{
    if( NULL == m_pdds )
        return E_POINTER;
    ...
    return m_pdds->Blt( &rec, pdds, prc, dwFlags,NULL );
}
...
// Desc: 
//-----------------------------------------------------------------------------
HRESULT CSurface::Blt( DWORD x, DWORD y, DWORD Width, DWORD Height, CSurface* pSurface, RECT* prc )
{
    if( NULL == pSurface )
        return E_INVALIDARG;
